...
public class ProfessorAccountController implements Observer<GradeChangeEvent> {
            ...
            for (Student student : studenti) {
                for (Tema tema : teme) {
                    ...
                    if(grades.stream().noneMatch(gr -> gr.getId().equals(nid))){
                        ...
                        todo: change this constructor so that we add the date//Nota nota = new Nota(student.getId(), tema.getId(), valoare,this.loggedInProfessor.getNume()+" "+this.loggedInProfessor.getPrenume(),feedback);
                        fictionalGrades.add(new NotaDTO(nota,tema,student));
    ...
    //assert(false)
    //todo: do debug -> find out why grade does not set ok after new year
            //todo:  -> find out why NullPointerException + invocationtargetexcedption... (when working with grades etc)
    ...
    private void filterMergeSearchNota() {
            ...
            for (Student student : studenti) {
                for (Tema tema : teme) {
                    ...
                    if(grades.stream().noneMatch(gr -> gr.getId().equals(nid))){
                        int valoare = computeMaxGrade(student, tema, this.dateNotaData.getValue().format(Constants.DATE_TIME_FORMATTER));
                        String feedback = computeFeedback(student, tema, this.dateNotaData.getValue().format(Constants.DATE_TIME_FORMATTER));
                        //Nota nota = new Nota(student.getId(), tema.getId(), valoare,this.loggedInProfessor.getNume()+" "+this.loggedInProfessor.getPrenume(),feedback);
        ...
        this.tableNota.getSelectionModel().clearSelection();
    ...
    public void handleDeleteNota(ActionEvent actionEvent) {
        if(!checkboxInvert.isSelected()){
            NotaDTO n = this.tableNota.getSelectionModel().getSelectedItem();
            if(n == null){
                StudentAlert.showMessage(null, Alert.AlertType.ERROR,"Eroare","selectati o nota din tabel!");
            }
            else{
                this.service.removeByIdNota(n.getIdNota());
...
//TODO: lucrul cu notele                                                                                    (V)
//TODO: filtrari pentru nota (si ce cui mai trebui)                                                         (V)
//TODO: (AUTO) FUNCTION: compute grade                                                                      (V)
//TODO: (AUTO) FUNCTION: compute feedback                                                                   (V)
//TODO: (AUTO) FUNCTION (EXTRA FORM): add motivations (recompute grade considering motivations)
//TODO: (AUTO) FUNCTION: compute grade ON date change                                                       (V)
//TODO:------------------------------
//TODO: ^upper^ - !!! WHEN NEW FUTURE GRADE IS CREATED !!! (best time to compute) !!!                       (V)
//TODO:------------------------------
//TODO: AUTO: on select grade from table AND INVERT is on - fill textfields with entity attributes          (V)
//TODO: update + RESTRICTIONS                                                                               (V)
//TODO: delete + RESTRICTIONS                                                                               (V)